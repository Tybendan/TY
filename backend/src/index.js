...
app.get('*', (_req, res) => {
  ...
  try {
    if (existsSync(indexPath)) {
      ...
      res.status(500).send(`index.html not found at ${indexPath}. Public dir contents: ${JSON.stringify(existsSync(frontendDist) ? readdirSync(frontendDist) : 'NOT FOUND')}`);
    }
  } catch (err) {
...
app.listen(PORT, '0.0.0.0', () => {
  console.log(`[backend] 鉁  Server running on http://0.0.0.0:${PORT}`);
  console.log(`[backend] 鉁  Health check: http://0.0.0.0:${PORT}/api/health`);
});